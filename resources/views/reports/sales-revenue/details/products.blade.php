@extends('layouts.app')
@section('page-title', 'Seluruh Produk Terjual')
@section('content')
...
<div class="mx-auto max-w-6xl">
    ...
    <div class="card-feature mb-6">
        <div class="p-5 border-b border-[var(--color-hairline-soft)] flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h2 class="type-subtitle-lg text-[var(--color-ink-deep)]">Seluruh Produk Terjual</h2>
            ...
            <form method="GET" action="{{ route('reports.sales_revenue.products') }}" class="flex flex-col md:flex-row md:items-center gap-3 w-full">
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead><tr class="bg-[var(--color-surface-soft)]">
                    ...
                    <th class="px-5 py-3 text-left table-header">Kode Produk</th>
                    <th class="px-5 py-3 text-left table-header">Nama Produk</th>
                ...
                <tr class="border-b border-[var(--color-hairline-soft)]/50 hover:bg-[var(--color-surface-soft)] transition-colors">
                    <td class="px-5 py-3 type-body-sm text-[var(--color-slate)]">{{ $products->firstItem() + $idx }}</td>
                    ...
                    <td class="px-5 py-3 type-body-sm-bold text-right text-[var(--color-ink-deep)]">{{ number_format($p->total_qty, 0, ',', '.') }}</td>
                    <td class="px-5 py-3 type-body-sm-bold text-right text-[var(--color-ink-deep)]">Rp {{ number_format($p->total_revenue, 0, ',', '.') }}</td>
                </tr>
                @empty
                <tr><td colspan="5" class="px-5 py-10 text-center type-body-sm text-[var(--color-slate)]">Tidak ada produk terjual pada periode ini.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
