...
{{-- ═══════════════════════════════════════════════════════ --}}
{{-- 2. TOP PRODUK TERJUAL                                   --}}
{{-- ═══════════════════════════════════════════════════════ --}}
<div class="card-feature mb-6 flex flex-col min-h-[500px]" x-data="{ view: 'chart', chartType: 'bar', metric: 'qty' }" x-init="$watch('chartType', val => updateTopProductsChart(val, metric)); $watch('metric', val => updateTopProductsChart(chartType, val))">
    <div class="flex flex-wrap items-center justify-between gap-4 p-5 border-b border-[var(--color-hairline-soft)]">
        <div>
            <h2 class="type-subtitle-lg text-[var(--color-ink-deep)]">{{ __('messages.top_selling_products') }}</h2>
            <p class="type-caption text-[var(--color-slate)]">Berdasarkan jumlah unit terjual (Qty)</p>
        </div>
    ...
    <div x-show="view === 'table'" x-cloak class="flex-1 flex flex-col"><div class="overflow-x-auto flex-1">
        <table class="w-full">
            <thead><tr class="bg-[var(--color-surface-soft)]">
                ...
                <th class="px-5 py-3 text-left table-header">Produk</th>
            ...
            <tr onclick="window.location='{{ route('products.show', $tp->product_code) }}'" class="border-b border-[var(--color-hairline-soft)]/50 hover:bg-[var(--color-surface-soft)] transition-colors cursor-pointer">
                <td class="px-5 py-3">
                    @if($idx < 3)
                        <span class="type-caption-bold text-[var(--color-primary)]">{{ ['🥇','🥈','🥉'][$idx] }}</span>
                    @else
                        <span class="type-body-sm text-[var(--color-slate)] font-bold">{{ $idx + 1 }}</span>
                    @endif
                </td>
                <td class="px-5 py-3 type-body-sm font-medium text-[var(--color-ink)]">{{ $tp->name }}</td>
                <td class="px-5 py-3 text-right"><span class="badge badge-info">{{ number_format($tp->total_qty, 0, ',', '.') }}</span></td>
                <td class="px-5 py-3 type-body-sm text-right text-[var(--color-ink)]">Rp {{ number_format($tp->total_revenue, 0, ',', '.') }}</td>
                <td class="px-5 py-3 text-right"><span class="type-caption-bold text-[var(--color-primary)]">{{ $tp->revenue_pct }}%</span></td>
            </tr>
            @empty
            <tr><td colspan="5" class="px-5 py-10 text-center type-body-sm text-[var(--color-slate)]">Belum ada data penjualan.</td></tr>
            @endforelse
            </tbody>
        </table>
        
        </div><div class="p-4 border-t border-[var(--color-hairline-soft)] text-center mt-auto">
            <a href="{{ route('reports.sales_revenue.products', request()->all()) }}" class="bg-[var(--color-ink)] text-white hover:bg-[var(--color-ink-deep)] transition-colors rounded-[var(--radius-full)] font-medium type-body-sm !py-2 !px-6 inline-flex items-center gap-2">
                Lihat Keseluruhan Data
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>
            </a>
        </div>
...
{{-- ═══════════════════════════════════════════════════════ --}}
{{-- 3. TOP KATEGORI PENJUALAN                               --}}
{{-- ═══════════════════════════════════════════════════════ --}}
<div class="card-feature mb-6 flex flex-col min-h-[500px]" x-data="{ view: 'chart', chartType: 'doughnut', metric: 'revenue' }" x-init="$watch('chartType', val => updateCategoryChart(val, metric)); $watch('metric', val => updateCategoryChart(chartType, val))">
    <div class="flex flex-wrap items-center justify-between gap-4 p-5 border-b border-[var(--color-hairline-soft)]">
        <div>
    ...
    <div x-show="view === 'table'" x-cloak class="flex-1 flex flex-col"><div class="overflow-x-auto flex-1">
        <table class="w-full">
            ...
            <a href="{{ route('reports.sales_revenue.categories', request()->all()) }}" class="bg-[var(--color-ink)] text-white hover:bg-[var(--color-ink-deep)] transition-colors rounded-[var(--radius-full)] font-medium type-body-sm !py-2 !px-6 inline-flex items-center gap-2">
                Lihat Keseluruhan Data
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>
            </a>
        </div>
...
window.rawChartData = {
    ...
    topProducts: @json($topProducts->values()),
    categories: @json($categoryBreakdown->values()),
