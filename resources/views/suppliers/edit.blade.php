    ...
    <form method="POST" action="{{ route('suppliers.update', $supplier) }}" class="space-y-5" enctype="multipart/form-data">@csrf @method('PUT')
        ...
        <div>
            ...
            <div class="flex gap-2">
                ...
                <input type="text" name="phone_number" value="{{ old('phone_number', $supplier->phone_number) }}" minlength="7" maxlength="20" class="input-field flex-1" placeholder="81234567890">
            </div>
        </div>
        ...
        <div><label class="block type-body-sm-bold text-[var(--color-ink)] mb-1.5">{{ __('messages.address') }}</label><textarea name="address" rows="3" maxlength="300" class="input-field !h-auto !py-3">{{ old('address', $supplier->address) }}</textarea><p class="mt-1 text-xs text-gray-400">Maks. 300 karakter</p></div>
        <div>
