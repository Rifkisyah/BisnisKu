@extends('layouts.app')
@section('page-title', __('messages.edit') . ' ' . __('messages.procurement'))
@section('content')
<div class="mx-auto max-w-4xl" x-data="purchaseForm()">
    <div class="bg-[var(--color-canvas)] rounded-[32px] border border-gray-100 p-8 shadow-sm">
        <form method="POST" action="{{ route('product-purchases.update', $productPurchase) }}" class="space-y-8">@csrf @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                <div><label class="block type-body-sm-bold text-[var(--color-ink)] mb-1.5">{{ __('messages.purchase_date') }} *</label>
                    <input type="date" name="purchase_date" value="{{ old('purchase_date', $productPurchase->purchase_date->format('Y-m-d')) }}" required class="input-field"></div>
                <div><label class="block type-body-sm-bold text-[var(--color-ink)] mb-1.5">Estimasi Tiba (Opsional)</label>
                    <input type="date" name="estimated_arrival_date" value="{{ old('estimated_arrival_date', $productPurchase->estimated_arrival_date ? $productPurchase->estimated_arrival_date->format('Y-m-d') : '') }}" class="input-field"></div>
                <div><label class="block type-body-sm-bold text-[var(--color-ink)] mb-1.5">{{ __('messages.notes') }}</label>
                    <input type="text" name="notes" value="{{ old('notes', $productPurchase->notes) }}" maxlength="500" class="input-field" placeholder="Catatan umum..."></div>
            </div>
...
function purchaseForm() {
    return {
        @php
            $oldItems = old('items');
            if (!$oldItems) {
                $oldItems = $productPurchase->items->map(function($item) {
                    return [
                        ...
                        'marketplace_seller' => $item->marketplace_seller ?? '',
                        'marketplace_notes' => $item->marketplace_notes ?? '',
