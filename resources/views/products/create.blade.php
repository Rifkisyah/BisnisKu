...
<div class="mx-auto max-w-2xl"><div class="bg-[var(--color-canvas)] rounded-[32px] border border-gray-100 p-8 shadow-sm">
    <form method="POST" action="{{ route('products.store') }}" enctype="multipart/form-data" class="space-y-5">@csrf
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            ...
            <div><label class="block type-body-sm-bold text-[var(--color-ink)] mb-1.5">{{ __('messages.unit') }}</label><input type="text" name="unit" value="{{ old('unit', 'pcs') }}" placeholder="pcs, unit, set, kg..." minlength="1" maxlength="20" class="input-field"><p class="mt-1 text-xs text-gray-400">{{ __('messages.max_20_chars') }}</p></div>
            <div class="sm:col-span-2"><label class="block type-body-sm-bold text-[var(--color-ink)] mb-1.5">{{ __('messages.description') }}</label><textarea name="description" rows="3" maxlength="500" class="input-field !h-auto !py-3">{{ old('description') }}</textarea><p class="mt-1 text-xs text-gray-400">{{ __('messages.max_500_chars') }}</p></div>
