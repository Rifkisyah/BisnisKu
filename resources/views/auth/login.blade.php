...
<head>
    ...
    <title>{{ __('Log in') }} - {{ $globalShopName }}</title>
    <!-- Fonts -->
...
<div class="flex min-h-screen">
    ...
    <div class="hidden w-1/2 lg:flex items-center justify-center bg-[var(--color-surface-soft)] p-16">
        <div class="text-center max-w-lg">
            ...
            <h1 class="type-display text-[var(--color-ink-deep)]">{{ $globalShopName }}</h1>
            <p class="mt-4 type-subtitle-md text-[var(--color-slate)]">{{ $globalShopName }} {{ __('is the all-in-one Point of Sale and management platform. Designed for speed, built for growth, and powered by smart algorithms.') }}</p>
    ...
    <div class="flex w-full items-center justify-center px-6 lg:w-1/2 relative">
        ...
        <div class="w-full max-w-sm">
            <div class="mb-8 text-center lg:text-left">
                ...
                <h2 class="type-heading-lg text-[var(--color-ink-deep)]">{{ __('Welcome back') }}</h2>
                <p class="mt-2 type-body-md text-[var(--color-slate)]">{{ __('Please log in to your account.') }}</p>
            </div>
            ...
            <div class="mb-6 rounded-[var(--radius-lg)] border border-[var(--color-critical-strong)] bg-[var(--color-canvas)] p-4 shadow-[0_1px_2px_rgba(208,2,27,0.1)]">
                <p class="type-body-sm text-[var(--color-critical)]">{{ $errors->first() }}</p>
            </div>
            @endif
            ...
            <form method="POST" action="{{ route('login.submit') }}" class="space-y-6">
                ...
                <div x-data="{ show: false }">
                    ...
                    <div class="relative">
                        <input :type="show ? 'text' : 'password'" name="password" required
                               class="w-full h-11 bg-[var(--color-canvas)] text-[var(--color-ink)] border border-[var(--color-hairline)] rounded-[var(--radius-lg)] px-4 pr-10 type-body-sm transition-colors focus:border-[var(--color-primary)] focus:ring-1 focus:ring-[var(--color-primary)] outline-none" placeholder="••••••••">
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center">
                        <input type="checkbox" name="remember" id="remember" class="h-4 w-4 rounded-[var(--radius-xs)] border-[var(--color-hairline)] text-[var(--color-primary)] focus:ring-[var(--color-primary)]">
                        <label for="remember" class="ml-2 type-body-sm text-[var(--color-charcoal)]">{{ __('Remember me') }}</label>
                    </div>
                </div>
                <button type="submit" id="btn-login-submit" class="btn-buy w-full shadow-sm hover:-translate-y-0.5 transition-transform">
                    {{ __('Log In') }}
                </button>
            </form>
            
            <div class="mt-8 text-center">
                <p class="type-body-sm text-[var(--color-slate)]">{{ __("Don't have an account?") }} <a href="{{ route('register') }}" class="text-[var(--color-primary)] font-bold hover:underline">{{ __('Sign up') }}</a></p>
            </div>
        </div>
    </div>
</div>
