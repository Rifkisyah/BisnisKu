...
<div class="mx-auto max-w-lg"><div class="card-feature p-8">
    <form method="POST" action="{{ route('employees.update', $employee) }}" class="space-y-5">@csrf @method('PUT')
        ...
        <div x-data="{ show: false }">
            ...
            <div class="relative">
                <input :type="show ? 'text' : 'password'" name="password" minlength="8" class="input-field pr-10" placeholder="Kosongkan jika tidak diubah">
        ...
        <div><label class="block type-body-sm-bold text-[var(--color-ink)] mb-1.5">{{ __('messages.role') }} *</label>
            <select name="role_id" required class="input-field">@foreach($roles as $role)<option value="{{ $role->id }}" {{ $employee->role_id == $role->id ? 'selected' : '' }}>{{ $role->display_name }}</option>@endforeach</select></div>
    ...
    <a href="{{ route('employees.show', $employee) }}" class="btn-ghost w-full sm:w-auto text-center order-2 sm:order-1"><svg class="w-4 h-4 inline-block -mt-0.5 mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18" /></svg>{{ __('messages.back') ?? 'Kembali' }}</a>
